...
const ProgressBar = ({
  ...
  let percentageClasses = classnames('c-progressBar--progress');
  if (max <= percentage) percentageClasses += classnames(' c-progressBar--progress u-width-100percent');
  else {
    const sharePercentage = ceil((percentage % step) * (100 / step));
    if (min <= percentage) percentageClasses += classnames(` c-progressBar--progress u-width-${sharePercentage}percent`);
    else percentageClasses += classnames(' u-width-0percent');
  }
  return (
    <div className="c-progressBar u-width-100percent">
